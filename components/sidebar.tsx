...
import { canView, type NavigationKey } from "@/lib/access-control";

const links = [
  ["الرئيسية", LayoutDashboard, "/", "dashboard"], ["الخطة الاستراتيجية", Compass, "/strategy", "strategy"], ["المحاور", Milestone, "/axes", "axes"],
  ["الأهداف", Target, "/objectives", "objectives"], ["المبادرات", Lightbulb, "/initiatives", "initiatives"], ["المؤشرات", Gauge, "/kpis", "kpis"],
  ["الأقسام", Building2, "/departments", "departments"], ["المهام", ListChecks, "/tasks", "tasks"], ["التقارير", FileText, "/reports", "reports"],
  ["المخاطر والتحديات", ShieldAlert, "/risks", "risks"], ["الاعتمادات", BadgeCheck, "/approvals", "approvals"], ["التنبيهات", Bell, "/notifications", "notifications"],
  ["التكاملات", Plug, "/settings/integrations", "integrations"], ["المستخدمون", Users, "/users", "users"], ["الإعدادات", Settings, "/settings", "settings"],
] as const;

export function Sidebar({ role }: { role: Role }) {
  const visible = links.filter(([, , , key]) => canView(role, key as NavigationKey));
