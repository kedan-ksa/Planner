...
export default async function Dashboard() {
  const [axes, initiatives, kpiCount, updatedKpis, departments] = await Promise.all([
    ...
    db.initiative.findMany({
      ...
      take: 8,
      select: { id: true, title: true, departmentId: true, progress: true, status: true },
    }),
    db.kPI.count(),
  ...
  return <DashboardShell><div className="p-5 lg:p-8">
    ...
    <section className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <StatCard label="إجمالي المبادرات" value={String(initiatives.length)} detail={`${departments.length} إدارات وأقسام`} icon={Lightbulb} tone="blue" />
