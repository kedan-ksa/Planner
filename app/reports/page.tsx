...
import { createDepartmentReport, transitionReport } from "./actions";
...
export default async function ReportsPage() {
  ...
  return <DashboardShell><div className="p-5 lg:p-8"><div><h1 className="text-2xl font-bold">التقارير الدورية</h1><p className="text-sm text-slate-500">إنشاء تقرير الإدارة ومراجعته وإرساله واعتماده</p></div>
    ...
    <div className="mt-5 space-y-4">{reports.map((report)=><article key={report.id} className="card p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-bold">{report.title}</h2><p className="text-sm text-slate-500">{report.departmentId ? departmentNames.get(report.departmentId) ?? "إدارة" : "تقرير تنفيذي"}</p></div><span className="badge bg-blue-50 text-blue-700">{statusNames[report.status]}</span></div><p className="mt-4 text-sm">{report.summary ?? "بانتظار إعداد الملخص."}</p><div className="mt-4"><div className="mb-1 flex justify-between text-xs"><span>اكتمال التقرير</span><b>{report.completion}%</b></div><div className="h-2 rounded-full bg-slate-100"><div className="h-full rounded-full bg-teal-600" style={{width:`${report.completion}%`}}/></div></div><div className="mt-4 flex flex-wrap gap-2"><form action={transitionReport}><input type="hidden" name="reportId" value={report.id}/><button name="intent" value="start" className="rounded-lg border px-3 py-2 text-sm">بدء الإعداد</button> <button name="intent" value="ready" className="rounded-lg border px-3 py-2 text-sm">جاهز للمراجعة</button> {(user.role===Role.SUPER_ADMIN||user.role===Role.DEPARTMENT_MANAGER)&&<button name="intent" value="submit" className="rounded-lg bg-blue-600 px-3 py-2 text-sm text-white">إرسال للإدارة</button>} {canApprove&&<><button name="intent" value="approve" className="rounded-lg bg-emerald-700 px-3 py-2 text-sm text-white">اعتماد</button> <button name="intent" value="return" className="rounded-lg bg-amber-100 px-3 py-2 text-sm text-amber-800">إعادة للتعديل</button></>}</form></div></article>)}{reports.length===0&&<div className="card p-12 text-center text-slate-500">لا توجد تقارير في نطاقك بعد. أنشئ أول مسودة من الأعلى.</div>}</div>
  </div></DashboardShell>;
}
