...
export default async function TasksPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  const personalOnly = user.role === Role.DEPARTMENT_MEMBER || user.role === Role.VIEWER;
  const visible = await visibleDepartmentIds(user.role, user.organizationId, user.departmentId);
  const initiatives = await db.initiative.findMany({
    where: { department: { organizationId: user.organizationId! }, ...(personalOnly || visible === null ? {} : { departmentId: { in: visible } }) },
    select: { id: true, title: true, departmentId: true },
  });
  const account = await db.account.findFirst({ where: { userId: user.id, provider: "microsoft-entra-id" }, select: { providerAccountId: true } });
  const assignedExternal = personalOnly && account ? await db.externalEntity.findMany({
    where: { provider: "MICROSOFT_PLANNER", entityType: "TASK", payload: { path: ["assignments", account.providerAccountId], not: Prisma.AnyNull } },
    select: { externalId: true, payload: true },
  }) : [];
  const assignedIds = assignedExternal.filter((entry) => {
  ...
  const [externalTasks, members, recentUpdates] = await Promise.all([
  ...
  return <DashboardShell><div className="p-5 lg:p-8">
    <nav aria-label="صفحات المهام" className="mb-4 flex items-center gap-4 text-sm">
      <span>{total} مهمة · الصفحة {page}</span>
      {page > 1 && <Link href={`/tasks?page=${page - 1}`}>السابق</Link>}
      {page * 50 < total && <Link href={`/tasks?page=${page + 1}`}>التالي</Link>}
    </nav>
    <div className="flex items-center gap-3"><span className="rounded-xl bg-teal-50 p-3 text-teal-700"><ListChecks/></span><div><h1 className="text-2xl font-bold">المهام والتحديثات</h1><p className="text-sm text-slate-500">كل موظف يحدّث مهامه المسندة، والمدير يتابع مهام إدارته وسجل التغييرات · 50 مهمة لكل صفحة</p></div></div>
